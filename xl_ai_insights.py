...
Usage:
    xl-ai-insights [source ...] [--mirdash-base=URL] [--no-open] [--quiet] [--verbose]

    source        e.g. claude, codex, gemini — same as paxel.py (default: all)
    --mirdash-base=URL  override the mirdash server URL
    --no-open     skip opening the mirdash report URL in the browser
...
def main():
    ...
    wrapper_flags = {"--no-open", "--quiet", "--verbose"}
    ...
    # --- Step 1: Resolve mirdash base URL ---
    mirdash_base = _resolve_mirdash_base(argv)

    # --- Step 2: Authenticate BEFORE running paxel ---
    port = 8799
    redirect_uri = f"http://127.0.0.1:{port}/callback"
    auth_url = f"{mirdash_base}/cli-auth?redirect_uri={urllib.parse.quote(redirect_uri, safe='')}"
    # Request the right number of tokens for batch modes; default needs 1
    if token_count > 1:
        auth_url += f"&count={token_count}"

    if not quiet:
        print(f"\n  Opening mirdash for authentication… (close the browser or wait {_SHARE_AUTH_TIMEOUT}s to skip)")
    # Browser-open failure is a clean no-op, not an error: this is an optional share
    # command, so on a headless/CI/SSH box with no GUI browser it must skip without
    # failing the whole job (README documents "prints a warning and exits cleanly —
    # nothing is uploaded"), matching the auth-timeout path below.
    try:
    ...
    # =========================================================================
    # Default path: current month (month-to-date)
    # =========================================================================
    ...
    if summary is not None and not _summary_is_empty(summary):
        # Common case: current month has data — upload it
        if not quiet:
    ...
    # --- Fallback: current month is empty — find most recent month with data ---
    if not quiet:
    ...
    # Build the window for that fallback month and upload it
