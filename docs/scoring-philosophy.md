...
| Existing signals | Keep skills, MCPs, CLIs, fanout, output, delegation, and model diversity scored | They are educational prompts for capabilities users should learn, not claims of output quality. (ToolSearch was dropped from scoring in v17 — ~94% harness-forced `select:` tool-loading — and is now a published diagnostic only.) |
...
| Recency | Score the requested window once; no recency blend | A point should mean the span it labels. The old 65% recent / 35% full-window blend was written for a six-month window, where it damped one unusual month against five stable ones. At a one-month window its two components covered 93.3-100% of the same days, so it averaged a month with itself, and it published each axis's counts from the 30-day component while every total beside them stayed full-window. |
