...
def build_profile(stats, model_usage=None):
    ...
    # Every field below reads the SAME `stats["agentic"]`. Until v11 a corpus run stashed
    # the pre-blend AQ under `_full_window_agentic` and the identity/descriptive fields
    # (archetype, steering, signature moves) were derived from that copy so a 30-day
    # bucket could not rewrite them. There is no blend and therefore no second copy any
    # more: `stats["agentic"]` is the scoring-window AQ, full stop.
