"""The multi-source blend must read the SAME calibration constants the scorer does.

`aggregate.blend_model_mix_components` is on the live path (aggregate.py, the Model mix
branch of the per-axis combine), not a replay-only helper. It used to carry its own
copies of AQ's Model mix targets as bare literals. That is the exact defect the
calibration fingerprint exists to prevent, one module further out: a retune of
`MODELS_DISTINCT_CEILING` or `OFFLOAD_SHARE_TARGET` would move every single-source AQ
and the fingerprint, while the blended multi-source Model mix silently kept scoring
against the stale value -- publishing divergent numbers under a contract id that claims
they are comparable.
